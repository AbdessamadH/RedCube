...
class SplashActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val ubeContainer = findViewById<View>(R.id.ubeContainer)
        ...
        // Tilt "u" as if it continued the arc's curve, pivoting from its
        // base rather than its center so it reads as leaning, not spinning.
        textU.post {
            textU.pivotX = textU.width / 2f
            textU.pivotY = textU.height.toFloat()
            textU.rotation = -10f
        }

        // "red" appears, then "ube" a beat later: rhythm instead of a synced fade.
        fadeInView(textRed, delay = 1000)
        fadeInView(ubeContainer, delay = 1150)
    ...
    private fun fadeInView(view: View, delay: Long) {
        ...
        val translateY = PropertyValuesHolder.ofFloat(
            View.TRANSLATION_Y, view.resources.displayMetrics.density * 6f, 0f
        )
